...
from utils.contamination_metrics import get_or_create_baseline, compute_contamination_metrics
import time
import wandb
...
def parse_args():
    ...
    parser.add_argument("--device", type=str, default=default_device, help="Device to run the model on, e.g., 'cpu', 'mps', or 'cuda:0'. Auto-detected when omitted (previously hardcoded to 'cuda:0' -- see FORK_NOTES.md).")
    ...
    parser.add_argument("--min_p", type=float, default=None, help="Min-p sampling threshold for generating texts (disabled when omitted).")
...
def main():
    ...
    # Captured before tokenization/truncation below: the raw human-authored
    # text, used as the rare-token baseline's source (see FR-004 / US3).
    human_texts_for_baseline = list(dataset["text"])
    ...
    baseline = get_or_create_baseline(args.experiment_path, human_texts_for_baseline)
    contamination_metrics = compute_contamination_metrics(generated_texts, baseline)
