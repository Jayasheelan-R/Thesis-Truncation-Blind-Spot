...
try:
    ...
    # Removed in newer transformers versions; telemetry-only, safe to no-op.
...
def main():
    ...
    # This script calls wandb.log() directly (not only through Trainer's
    # report_to callback), so a run must exist in *this* process first --
    # generate.py already does this; train.py previously didn't, which
    # crashed with "You must call wandb.init() before wandb.log()" (see
    # FORK_NOTES.md, 2026-08-05 entry).
    wandb.init(name=training_args.run_name, config=vars(model_args) | vars(data_args))
