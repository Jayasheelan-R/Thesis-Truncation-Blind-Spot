...
from src.utils.audit import write_generation_config_audit
import wandb


def is_generation_complete(experiment_path, iteration):
    """A generation counts as complete once its trained model exists, and
    (for iteration >= 1, which also runs a generate.py step) its
    data_metrics.json exists too. Used to resume a multi-generation run
    without redoing or overwriting already-completed work (FR-010)."""
    model_dir = os.path.join(str(experiment_path), str(iteration), "model", "final_model")
    if not os.path.isdir(model_dir):
        return False
    if iteration == 0:
        return True
    metrics_path = os.path.join(str(experiment_path), str(iteration), "data_metrics.json")
...
def main(cfg: DictConfig):
    ...
    # bfloat16 model loading reproducibly segfaults on this MPS backend (see
    # FORK_NOTES.md, 2026-08-05 "bfloat16 + MPS" entry) -- use it only where
    # real CUDA is available (e.g. Colab); fall back to float32 explicitly
    # (reported below) everywhere else, rather than crashing. Applies
    # regardless of smoke_test, since config.yaml/ours.yaml both default
    # torch_dtype to bfloat16 for real runs too.
    if torch_dtype == "bfloat16" and not torch.cuda.is_available():
        torch_dtype = "float32"
    print(f"cuda_available={torch.cuda.is_available()} -> using torch_dtype={torch_dtype}")

    if cfg.smoke_test:
        num_samples = "100"
        num_iterations = 1
        low_cpu_mem_usage = str(True)

    # Initial training
    if is_generation_complete(experiment_path, 0):
        print("Skipping iteration 0: already completed (resuming).")
    else:
        subprocess.run([
            "python", "src/train.py",
    ...
    for iteration in range(1, num_iterations+1):
        ...
        if is_generation_complete(experiment_path, iteration):
            print(f"Skipping iteration {iteration}: already completed (resuming).")
            continue
        ...
        if cfg.decoding.get("min_p", None) is not None:
            command.extend(["--min_p", str(cfg.decoding.min_p)])
        command += [
            "--torch_dtype", torch_dtype,
            "--low_cpu_mem_usage", low_cpu_mem_usage,
            "--classify_text", "1",
